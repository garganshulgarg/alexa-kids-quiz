...
public abstract class AbstractIntentHandler implements RequestHandler {
    ...
    public Optional<Response> handle(HandlerInput input) {
    	...
    	PolicyFactory policyFactory = new HtmlPolicyBuilder()
    								.allowElements("break", "emphasis", "voice", "audio")
    								.allowAttributes("src").matching(Pattern.compile(".*")).onElements("audio")
    								.allowAttributes("time", "level", "name").globally()
    								.toFactory();
    	String speechText=getSpeechText(input);
    	String repromptText = getRepromptText(input);
    ...
    public String applyVoice2Speech(final String speechText) {
    	if(StringUtils.isNotEmpty(speechText)) {
    		return "<voice name=\""+ alexaVoice + "\">" +speechText + "</voice>" ;
    	}
	...
	protected String getSpeechText(HandlerInput input) {
		if(StringUtils.isEmpty(speachName))
		{
			return null;
		}
        ...
        return applyVoice2Speech(writer.toString()); 		
	}
	
	protected String getRepromptText(HandlerInput input) {
		if(StringUtils.isEmpty(repromptName))
		{
			return null;
		}
